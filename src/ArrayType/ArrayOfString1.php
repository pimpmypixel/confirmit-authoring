...
class ArrayOfString1 extends AbstractStructArrayBase
{
     ...
     * @param string[] $string
     * @return \Confirmit\Authoring\ArrayType\ArrayOfString1
     */
    public function setString(array $string = array())
    {
        foreach ($string as $arrayOfString1StringItem) {
            // validation for constraint: itemType
            if (!is_string($arrayOfString1StringItem)) {
                throw new \InvalidArgumentException(sprintf('The string property can only contain items of string, "%s" given', is_object($arrayOfString1StringItem) ? get_class($arrayOfString1StringItem) : gettype($arrayOfString1StringItem)), __LINE__);
            }
        }
